...
```{r}
1 - var(resid(models[[best_model_name]])) / var(d79$afd_prop)
```
...
## Median abolute error per model
...
r2_bestmodel <- 1 - var(d79$err_avg) / var(d79$afd_prop)
r2_bestmodel
...
wahlkreise_shp %>% 
  left_join(select(d79, area_nr, afd_prop), by = c("WKR_NR" = "area_nr")) %>% 
  #drop_na(afd_prop) %>% 
  ggplot() +
  geom_sf(aes(fill = afd_prop)) + 
  theme_void() +
...
ggsave("../img/best-model-pred-err.pdf")
```
